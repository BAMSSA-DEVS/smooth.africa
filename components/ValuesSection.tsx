      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {values.map((val, i) => (
            <motion.div
              key={val.id}
              className="group p-6 rounded-2xl bg-white dark:bg-[#1C1C1C] border border-stone-200 dark:border-[#2A2A2A] hover:border-stone-300 dark:hover:border-[#3A3A3A] hover:shadow-[0_4px_20px_rgb(0,0,0,0.05)] dark:hover:shadow-[0_4px_20px_rgb(0,0,0,0.3)] transition-all duration-300 space-y-4"
              initial={{ opacity: 0, y: 24 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.6, ease: [0.25, 0.46, 0.45, 0.94], delay: 0.1 + i * 0.08 }}
            >
              {/* Icon */}
              <div className="p-2.5 w-fit rounded-xl bg-stone-50 dark:bg-[#141414] border border-stone-100 dark:border-[#2A2A2A] text-stone-600 dark:text-stone-400 group-hover:text-[#1258AB] dark:group-hover:text-blue-400 group-hover:border-[#1258AB]/20 dark:group-hover:border-blue-500/20 transition-all">
